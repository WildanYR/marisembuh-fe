...
const emit = defineEmits(["update:modelValue", "update:modalShow"]);
...
const selectedItemIndex = computed(() => {
  if (!props.modelValue || !userList.value.length) return -1;
  return userList.value.findIndex(
...
const closeModal = () => {
  if (loadingAddUser.value) return;
