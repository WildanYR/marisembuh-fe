...
const addTherapyData = reactive({
  name: "",
});
const addTherapyDataError = reactive({
  name: [],
});

const loadingGetTherapy = ref(false);
const loadingAddTherapy = ref(false);
...
const closeModal = () => {
  if (loadingAddTherapy.value) return;
...
const handleAddTherapy = () => {
  ...
  validator.addValidation("name", addTherapyData.name, [isRequired]);
  if (validator.validate()) {
    addTherapyDataError.name = validator.getError("name") as any;
    return;
  }
  loadingAddTherapy.value = true;
  createTherapy({ ...addTherapyData })
    .then((response) => {
      if (!response) return;
      onSelectItem(response);
      addTherapyData.name = "";
      loadingAddTherapy.value = false;
      mode.value = "get";
      resetPaginationData();
      getTherapy();
    })
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <span>Tambah Riwayat Terapi</span>
                </button>
              </div>
              ...
              <template v-else>
                <template v-if="mode === 'add'">
                  <div class="flex flex-col items-center justify-center gap-4">
                    <TextInput
                      v-model="addTherapyData.name"
                      label="Nama Riwayat terapi"
                      :error-message="addTherapyDataError.name"
                      class="w-full"
                    ></TextInput>
                    <LoadingButton
                      :loading="loadingAddTherapy"
                      @click="handleAddTherapy"
                    ...
                    <GrayButton @click="onGetTherapyMode" class="w-full"
                      >batal</GrayButton
                    >
                  </div>
                </template>
                <template v-else>
                  <template v-if="therapyList.length">
                    <div
