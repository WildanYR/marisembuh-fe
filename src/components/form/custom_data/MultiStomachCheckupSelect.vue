...
const stomachcheckupList: Ref<IStomachCheckupResponse[]> = ref([]);
...
const addStomachCheckupData = reactive({
  name: "",
});
const addStomachCheckupDataError = reactive({
  name: [],
});

const loadingGetStomachCheckup = ref(false);
const loadingAddStomachCheckup = ref(false);
...
const isItemSelected = (id: number) => {
  if (!(props.modelValue as any)?.length || !stomachcheckupList.value.length)
    return false;
...
const closeModal = () => {
  if (loadingAddStomachCheckup.value) return;
...
const onSearch = debouncer.debounce((searchValue: string) => {
  ...
  if (!searchValue) {
    ...
    loadingGetStomachCheckup.value = true;
    getStomachCheckupByName(searchValue)
      .then((response) => {
        if (!response) return [];
        stomachcheckupList.value = response;
      })
      .finally(() => {
        loadingGetStomachCheckup.value = false;
      });
...
const onAddStomachCheckupMode = () => {
  mode.value = "add";
};
const onGetStomachCheckupMode = () => {
  mode.value = "get";
  resetPaginationData();
  getStomachCheckup();
};
...
const handleAddStomachCheckup = () => {
  ...
  validator.addValidation("name", addStomachCheckupData.name, [isRequired]);
  if (validator.validate()) {
    addStomachCheckupDataError.name = validator.getError("name") as any;
    return;
  }
  loadingAddStomachCheckup.value = true;
  createStomachCheckup({ ...addStomachCheckupData })
    .then((response) => {
      if (!response) return;
      onSelectItem(response);
      addStomachCheckupData.name = "";
      loadingAddStomachCheckup.value = false;
      mode.value = "get";
      resetPaginationData();
      getStomachCheckup();
    })
              ...
              <template v-else>
                <template v-if="mode === 'add'">
                  <div class="flex flex-col items-center justify-center gap-4">
                    <TextInput
                      v-model="addStomachCheckupData.name"
                      label="Nama Pemeriksaan Perut"
                      :error-message="addStomachCheckupDataError.name"
                      class="w-full"
                    ></TextInput>
                    <LoadingButton
                      :loading="loadingAddStomachCheckup"
                      @click="handleAddStomachCheckup"
                    ...
                    >
                      Tambah Pemeriksaan Perut
                    </LoadingButton>
                    <GrayButton @click="onGetStomachCheckupMode" class="w-full"
                      >batal</GrayButton
                    >
                  </div>
                </template>
                <template v-else>
                  <template v-if="stomachcheckupList.length">
                    <div
                      class="grid grid-cols-1 gap-3 lg:grid-cols-4 md:grid-cols-2"
                    >
                      <button
                        v-for="(item, i) in stomachcheckupList"
                        :key="id + '-' + i"
