...
const handleEditMedicine = () => {
  ...
  loadingSubmit.value = true;
  updateMedicine(parseInt(route.params.id as any), { ...formData })
    .then(() => {
      router.back();
    })
    .finally(() => {
...
const toPreviousPage = () => {
  router.push({ name: "MedicineList" });
};

onMounted(() => {
  if (route.params.id) {
    loadingGetMedicine.value = true;
    getMedicineById(parseInt(route.params.id as any))
      .then((response) => {
        if (!response) return;
