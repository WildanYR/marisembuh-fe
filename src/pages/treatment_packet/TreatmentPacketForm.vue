...
import {
  ...
  getTreatmentPacketById,
  updateTreatmentPacket,
...
const handleAddTreatmentPacket = () => {
  ...
  createTreatmentPacket({ ...formData })
    .then(() => {
      router.back();
    })
    .finally(() => {
...
const handleEditTreatmentPacket = () => {
  const validator = new Validator();
  validator.addValidation("name", formData.name, [isRequired]);
  if (validator.validate()) {
    formDataError.name = validator.getError("name") as any;
  ...
  loadingSubmit.value = true;
  updateTreatmentPacket(parseInt(route.params.id as any), { ...formData })
    .then(() => {
      router.back();
    })
    .finally(() => {
...
const handleSubmit = () => {
  if (route.name === "TreatmentPacketAdd") {
    handleAddTreatmentPacket();
  } else {
    handleEditTreatmentPacket();
  }
};

const toPreviousPage = () => {
  router.push({ name: "TreatmentPacketList" });
};

onMounted(() => {
  if (route.params.id) {
    loadingGetTreatmentPacket.value = true;
    getTreatmentPacketById(parseInt(route.params.id as any))
      .then((response) => {
        if (!response) return;
