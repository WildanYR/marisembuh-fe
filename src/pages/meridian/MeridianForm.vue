...
const handleEditMeridian = () => {
  ...
  loadingSubmit.value = true;
  updateMeridian(parseInt(route.params.id as any), { ...formData })
    .then(() => {
      router.back();
    })
    .finally(() => {
...
const toPreviousPage = () => {
  router.push({ name: "MeridianList" });
};

onMounted(() => {
  if (route.params.id) {
    loadingGetMeridian.value = true;
    getMeridianById(parseInt(route.params.id as any))
      .then((response) => {
        if (!response) return;
