...
const handleEditTongueCheckup = () => {
  ...
  loadingSubmit.value = true;
  updateTongueCheckup(parseInt(route.params.id as any), { ...formData })
    .then(() => {
      router.back();
    })
    .finally(() => {
...
const handleSubmit = () => {
  if (route.name === "TongueCheckupAdd") {
    handleAddTongueCheckup();
  } else {
    handleEditTongueCheckup();
  }
};

const toPreviousPage = () => {
  router.push({ name: "TongueCheckupList" });
};

onMounted(() => {
  if (route.params.id) {
    loadingGetTongueCheckup.value = true;
    getTongueCheckupById(parseInt(route.params.id as any))
      .then((response) => {
        if (!response) return;
