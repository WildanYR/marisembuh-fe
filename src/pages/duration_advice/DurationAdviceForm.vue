...
import {
  ...
  getDurationAdviceById,
  updateDurationAdvice,
...
const handleEditDurationAdvice = () => {
  const validator = new Validator();
  validator.addValidation("name", formData.name, [isRequired]);
  if (validator.validate()) {
    formDataError.name = validator.getError("name") as any;
...
const handleSubmit = () => {
  if (route.name === "DurationAdviceAdd") {
    handleAddDurationAdvice();
  } else {
    handleEditDurationAdvice();
  }
};
...
onMounted(() => {
  if (route.params.id) {
    ...
    getDurationAdviceById(parseInt(route.params.id as any))
      .then((response) => {
        if (!response) return;
